...
class SOAP(torch.optim.Optimizer):
    ...
    Arguments:
        ...
        precondition_1d: Whether to precondition 1D gradients or not.
        precondition_warmup: The number of initial steps for which the preconditioner is always updated.
        max_precond_size: The maximum size for the preconditioner. If a dimension exceeds this size, it is not preconditioned.
        merge_dims: Whether to merge gradient dimensions. For example, a gradient of shape (256, 256, 3, 3)
                    would become (256, 2304), keeping the first dimension separate as the output dimension.
    ...
    def init_preconditioner(
        ...
        r"""Initializes the preconditioner matrices."""
    ...
    def project(self, grad, state, back: bool = False):
        r"""Projects the gradient to/from the eigenbasis of the preconditioner."""
    ...
    def get_orthogonal_matrix_QR(self, state):
        r"""Computes the eigenbasis of the preconditioner using one round of power
